...
export default function SantriHistoryPage({ onNavigate, santriId, mode = 'learning', returnPath }: SantriHistoryPageProps) {
  ...
  // Fetch surah data for edit validation
  useEffect(() => {
    if (isWorship) return;
    fetch('/api/master-data?type=surahs')
      .then(r => r.json())
      .then(j => {
  ...
  const handleSaveEdit = async () => {
    ...
    // Validation for QURAN
    if (editRecord.type === 'QURAN' && editMaxVerse !== null) {
      const ep = parseInt(editForm.end_point);
      const sp = parseInt(editForm.start_point);
      if (!isNaN(ep) && ep > editMaxVerse) {
        toast.error(`Ayat akhir melebihi total ayat surah ini (maks: ${editMaxVerse})`);
        return;
      }
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      const res = await fetch(`/api/learning-records?id=${deleteId}`, { method: 'DELETE' });
      const json = await res.json();
      if (json.success) {
              ...
              <div key={record.id} className="bg-white p-4 rounded-xl border border-slate-100 shadow-sm">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {/* Edit & Delete buttons — only for learning records */}
                    {!isWorship && (
