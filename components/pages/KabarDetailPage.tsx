...
import { ArrowLeft, Edit2, X, Share2 } from 'lucide-react';
...
export default function KabarDetailPage({ onNavigate, postId, currentUser, fromPublic = false }: KabarDetailPageProps) {
    ...
    const canEdit = !!post && (String(currentUser?.id) === String(post.author_id) || currentUser?.role === 'admin');
    ...
    return (
        <div className="bg-white min-h-screen pb-20">
            ...
            <div className="sticky top-0 bg-white z-10 p-4 border-b border-slate-100 flex items-center justify-between">
                ...
                {canEdit && (
                    <button
                        ...
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold bg-slate-100 text-slate-700 hover:bg-slate-200 transition-colors"
                    >
                        <Edit2 size={14} />
                        Edit
                    </button>
            ...
            <div className="p-4 max-w-2xl mx-auto">
                ...
                {post.images && post.images.length > 0 && (
                    <div className="mb-6 group relative">
                        <div
                            ...
                            className="flex overflow-x-auto snap-x snap-mandatory bg-slate-100 rounded-lg [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden"
                        >
                            {post.images.map((img, idx) => (
                                <div
                                    key={idx}
                                    className="snap-center shrink-0 w-full h-[280px] sm:h-[400px] relative cursor-pointer"
                ...
                <div className="text-slate-700 leading-relaxed whitespace-pre-wrap text-sm md:text-base mb-6">
                    {post.content}
                </div>

                <button
                    onClick={handleShare}
                    className="w-full flex items-center justify-center gap-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-700 py-3 rounded-xl text-sm font-bold transition-colors"
                >
                    <Share2 size={16} />
                    Bagikan ke WA
                </button>
            ...
            <KabarPostModal
                ...
                onSuccess={(updated) => {
                    setPost((prev) => (prev ? { ...prev, title: updated.title, content: updated.content, images: updated.images } : prev));
                    setShowEditModal(false);
                    toast.success('Kabar berhasil diperbarui.');
                }}
            />
